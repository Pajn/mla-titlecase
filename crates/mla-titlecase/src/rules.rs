...
use crate::context::{
    first_significant_word, followed_by_hyphen, follows_colon, last_significant_word,
    likely_adverbial_particle, likely_name_particle_context, part_of_hyphenated_compound,
    preceded_by_hyphen, precedes_colon,
};
use crate::lexicon::{
...
pub(crate) fn apply(tokens: &[Token<'_>], options: &TitleCaseOptions<'_>) -> String {
    ...
    while index < tokens.len() {
        ...
        if should_force_lowercase(&key, should_capitalize, tokens, index, options) {
            // Small words and name particles matched against Latin-script
            // function-word lists, so they lowercase the English way. Locale
            // casing (e.g. Turkish dotless `ı`) applies only to genuine title
            // words, keeping "IN" -> "in" rather than "ın" under the Turkish
            // profile.
